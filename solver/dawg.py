...
class Dawg:
    ...
    def trace_string(self, string, start_node):
        current_node = start_node
        for char in string:
            if char.upper() in current_node.edges:
                current_node = current_node.edges[char.upper()]
            else:
                return None
        return current_node
    
    def words_containing_segs(self, rack, segs, node=None, prefix=''):
        words = []
        if node is None:
            node = self.root
        if node.final:
        ...
        for char in set(rack):
            if char in node.edges:
                next_rack = rack.replace(char, '', 1)
                words.extend(self.words_containing_segs(next_rack, segs, node.edges[char], prefix + char))
        if '?' in rack:
            next_rack = rack.replace('?', '', 1)
            for edge, child_node in node.edges.items():
                words.extend(self.words_containing_segs(next_rack, segs, child_node, prefix + edge.lower()))
        for seg in segs:
            next_node = self.trace_string(seg, node)
            if next_node:
                index = segs.index(seg)
                words.extend(self.words_containing_segs(rack, segs[index+1:], next_node, prefix + seg))
        return words
...
if __name__ == "__main__":   
    ...
    with open("csw.pickle", "wb") as output_file:
        # Save the object to the file using pickle.dump
        pickle.dump(dawg, output_file, pickle.HIGHEST_PROTOCOL)
